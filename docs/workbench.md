...
Official write gate: DSH CLI **`0.1.5-rc.1`**. The plugin peers on SDK `0.1.5-rc.2`; that is not “CLI 2”. Isolated candidate builds are a verification channel for this repo, not a user tutorial for bypassing the gate.
...
- Official CLI `0.1.5-rc.2` support
...
$root = "D:\dsh-workbench-dev"
$workbenchHome = "$root\home"
$tools = "$root\tools"
$artifacts = "$root\artifacts"
$snapshots = "$root\snapshots"
New-Item -ItemType Directory -Force -Path $workbenchHome, $tools, $artifacts, $snapshots | Out-Null
...
# --bin must be the selected DSH CLI bin.js whose adjacent package.json is 0.1.5-rc.1
...
| `--bin` (alias `--cli`) | Bound DSH `bin.js`. Write gate is `0.1.5-rc.1`. |
...
Do **not** install `@dsh-spaces/plugin` onto an ordinary space (including a profile named `coding`) as a way to “get Spaces.” That is a misinstall. The space then only shows **返回工作台** / **Return to workbench**.
...
Runtime **预览安装** / **预览升级** still go through the `0.1.5-rc.1` gate. Final upgrade and crash-recovery acceptance is not finished.
...
- Do not `dsh --profile coding` + `dsh plugin add @dsh-spaces/plugin` as the new architecture.
...
正式写门禁：DSH CLI **`0.1.5-rc.1`**。插件 peer 的 SDK `0.1.5-rc.2` 不是「CLI 2」。仓库里的 candidate 构建只是隔离验证通道，不是给用户绕门禁的教程。
...
# --bin 必须是相邻 package.json 为 0.1.5-rc.1 的 DSH bin.js
node packages/supervisor/lib/index.js `
  --home $workbenchHome `
  --bin D:\path\to\@deepseek-ai\dsh\lib\bin.js `
  --node "$env:ProgramFiles\nodejs\node.exe" `
...
| `--bin`（别名 `--cli`） | 绑定的 DSH `bin.js`，写门禁 `0.1.5-rc.1` |
...
不要把 `@dsh-spaces/plugin` 装到普通空间（包括名叫 `coding` 的 profile）来「获得 Spaces 管理」。那是误装。该空间只会显示 **返回工作台**。
...
运行时 **预览安装** / **预览升级** 仍受 `0.1.5-rc.1` 门禁。最终升级和崩溃恢复验收还没结束。
...
- 不要用 `dsh --profile coding` 再 `dsh plugin add @dsh-spaces/plugin` 当新架构的安装路径。
