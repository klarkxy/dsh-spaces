...
`--home` is required. `--cli` must be an absolute path to a bound DSH CLI. `verify`, `recover`, and `rollback` accept DSH CLI `0.1.5-rc.1` and `0.1.5-rc.2`. Unknown versions are refused.
