...
import type { BoardCommand, BoardMutationReceipt, CatalogEntry, DashboardError, InstanceRef, InstanceView, Placement, WidgetContent } from '../../../src/shared/dashboard.js';
...
function PlacementEditor({ placement, disabled, onChange }: { placement: Placement; disabled: boolean; onChange(position: Pick<Placement, 'x' | 'y' | 'w' | 'h'>): void }) {
  ...
  return <details className="dd-placement"><summary>调整布局</summary><form onSubmit={event => { event.preventDefault(); onChange(position); }}>
    {(['x', 'y', 'w', 'h'] as const).map(key => <label key={key}>{({ x: '列', y: '行', w: '宽', h: '高' })[key]}<input type="number" step={1} min={key === 'w' || key === 'h' ? 1 : 0} max={key === 'x' ? 11 : key === 'w' ? 12 : 10000} value={position[key]} disabled={disabled} onChange={event => setPosition({ ...position, [key]: event.currentTarget.valueAsNumber })} /></label>)}
    ...
    {frame && <div className="dd-body"><main className="dd-main">
      ...
      <div className="dd-grid">{frame.board?.placements.map(placement => {
        ...
        return <article key={placement.id} className="dd-card" style={{ gridColumn: `${placement.x + 1} / span ${placement.w}`, gridRow: `${placement.y + 1} / span ${placement.h}` }}>
          ...
          {view?.state === 'present' ? <CardBoundary key={`${view.runId}:${view.sequence}`}><WidgetContentView content={view.instance.content} /></CardBoundary> : <p>此处仅保留摆放位置，未启动来源或读取其私有文件。</p>}
      ...
      {frame.catalog.map(entry => <div className="dd-catalog-entry" key={refKey(entry.ref)}><div><strong>{entry.title}</strong><small>{entry.ref.providerId} · {entry.kind}</small></div><button type="button" disabled={!writable || !onPin} onClick={() => onPin?.(entry)} aria-label={`固定 ${entry.title}`}>固定</button></div>)}
...
export function DashboardApp({ backend, onOpen }: { backend: DashboardReadBackend; onOpen?(ref: InstanceRef): void }): ReactNode {
  ...
  useEffect(() => {
    generation.current++; setError(null); setWriteError(null); writing.current = false; setBusy(false);
    const session = new DashboardReadSession({
      read: signal => readDashboardFrame(backend, signal, boardId),
      onValue: value => setFrame(value), onFailure: failure => setError(failure), clearSensitive: () => setFrame(null),
    });
    session.start();
    return () => { generation.current++; session.close(); };
  }, [backend, boardId]);
  async function write(make: (current: DashboardFrame) => BoardCommand): Promise<void> {
    ...
    writing.current = true; setBusy(true); setWriteError(null);
    ...
    try {
      ...
      if (receipt.result.kind === 'board.saved') {
      ...
      if (identity === generation.current) setWriteError({ ...publicError(failure), requestId });
    } finally {
      if (timeout !== undefined) clearTimeout(timeout);
      if (identity === generation.current) { writing.current = false; setBusy(false); }
    }
  }
