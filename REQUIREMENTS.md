...
2. 重启只影响该 profile 的进程，其他 profile 与 GUI 不受影响。
3. Hub 退出时优雅关闭全部子进程（可配置：退出即停 / 保持后台）。
4. 进程崩溃要被侦测并在栏上显示异常态，提供一键重启。
...
| # | 义务 | 具体要求 |
|---|---|---|
| NFR1 | 永不损坏用户已有 home | `web` 永不打 patch；任何写 `cordis.patch.yml` 前先备份原文件（如 `cordis.patch.yml.bak-<时间戳>`）；写文件必须原子（临时文件+rename） |
| NFR2 | 版本漂移防护 | 每次启动某工作台前跑 `dsh --profile <name> --dump-config`，校验 `session-persistence-jsonl` 和 `storage-json` 两个 row id 仍存在且 patch 生效；校验失败 → **拒绝启动**并给出清晰提示，绝不静默用不安全配置起进程 |
