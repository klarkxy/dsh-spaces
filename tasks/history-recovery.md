...
本文记录的是从旧电脑找回项目代码的来源、分支和过程，**不是**产品故障恢复功能。标题里的「恢复」不得当作现行 Spaces 产品能力。下文「当前有效需求」改为当时需求，已被 let it crash 取代。
...
## 当时需求（2026-09-13 故障前；已被 2026-09-15 let it crash 取代）

原报告提出共享核心、Web 插件、薄桌面壳和 doctor CLI。后续已批准的工作台方案扩展并取代其早期 MVP 范围。其中快照与配置恢复、带外恢复、独立恢复 CLI 不再是现行施工与发布门槛：
...
Web 验证确认独立管理器启动、固定空间栏、coding/notes 启动并嵌入真实 DSH、普通空间只带轻量桥，以及管理器停止后稳定入口和 API 仍可用（当时页面文案为救援页；救援作为产品能力已撤销）。截图中的嵌入空间有真实 DSH 首次引导框；没有测试模型对话或声称完整主题/维护矩阵通过。Electron 测试为 packaged: false，未重新制作或验证安装器。
...
1. 原 6247b22 Git 提交对象/完整提交图、旧录像与配音、原隔离 Home 没有从旧电脑取回；取回的是有日志和快照证据的源码与需求状态。这是历史代码找回的缺口，不是产品故障恢复待办。
2. 故障前未完成的管理包**回滚 / 冷恢复**：**已撤销**（2026-09-15），不再另行推进。仍可后续推进且不含恢复：管理包实际安装/升级（失败不回退）、旧管理界面升级入口、运行时提交阶段故障注入（改验失败可见且无抢救）、rc.2 完整兼容验证。
3. 完整主题和浏览器失败场景、新演示录像、安装器验收仍未由本次基础运行验证覆盖。未登记外部 DSH 的发现仍是已知缺口；**不**新增恢复接管。
