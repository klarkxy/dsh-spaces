...
Isolation display, capability gates, sanitized errors, auth, and host-denial remain. **Restore-core, independent rescue, restore CLI, Doctor recover/unlock/rollback, snapshot restore UI, and fail-rollback are not current requirements** (**已撤销**). Remaining runtime work is R1–R6, pending.
