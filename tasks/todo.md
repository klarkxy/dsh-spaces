...
- [x] A0：固定最新基线与本地备份引用，完成独立测试基线（1 项可选官方 CLI 未跑，另列为后续真实验收）。
- [x] A1：279 个差异路径逐项审计，加核 3 个导入点独有差异；4 处文档/验收脚本遗漏已补齐，2 处 Windows 测试问题已独立复测。
- [ ] A2：审计收口后仅连接旧分支历史，验证树与祖先关系。
- [ ] B1：统一协议、权限、实例身份与正常升级交接合同。
- [ ] B2：后台用例、任务与管理写入收敛到现有 Supervisor。
- [ ] B3：模板、分享、批量、设置、插件缓存和诊断能力完整接入统一接口。
- [ ] B4：桌面使用共享工作台与后台，退出旧业务装配。
- [ ] B5：统一制品、受控升级和遗留代码清理。
- [ ] B6：跨端、故障、安装包及重要集成审查通过，准备本地合入。
