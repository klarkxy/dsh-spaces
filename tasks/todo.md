...
- [ ] 02. 统一插件状态、精确版本下载和多版本缓存；读不到实际版本显示未知；下载失败报错，不改用 latest。
- [ ] 03. 工作台/桌面展示安装、启动、崩溃错误（全项目一致）；无修复建议或恢复入口；空间可保持崩溃。
...
- [ ] 07. 普通空间导出插件组成、顺序及可选配置，未知来源明示。不是整 Home 灾难恢复。
- [ ] 08. 新空间暂存导入。三个结果分开：空间定义 / 插件安装 / 空间启动。不能要求启动成功才允许空间存在。失败只清理本次未发布且能证明归属的临时资源。
- [ ] 09. 补齐桌面入口，完成往返验收；合法但不兼容组合启动失败必须可见，不得修好后才算成功。

### M3：批量与模板

- [ ] 10. 批量逐空间执行；首项失败结束该批次（A 保留、B 失败、C 未执行）。无回滚、重试、自动继续或中断续接。
- [ ] 11. 新空间模板复用创建流程；已有空间不自动变化；模板导致运行失败只报错。
