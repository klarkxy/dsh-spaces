...
> **唯一活动任务账本是 [tasks/todo.md](tasks/todo.md)。** 本文件是导航和历史摘要，不再维护另一份互相矛盾的未完成列表。
>
...
> 迷路时看本文件末尾的「决策规则」和 `docs/let-it-crash.md`。
...
- [x] 新建向导 Dialog：名字校验 → 图标选择（预置 lucide 图标集）→ 分步进度（创建/装插件/写 patch/校验），失败展示错误（历史实现曾提供重试；现行政策下该次失败即结束，用户另发起是新请求）
...
- [x] 3.4 启动超时、端口探活失败、进程崩溃的用户可见错误提示；用户可从常规菜单主动重启（独立操作）。历史文案中的“崩溃恢复 / 一键重启”不再作为恢复产品要求
...
8. **let it crash**：报告真实失败，不要恢复它。不要用 catch、默认值、重试、补偿掩盖故障。不要把文档完成当成运行时迁移完成。现行任务只看 [tasks/todo.md](tasks/todo.md)。
