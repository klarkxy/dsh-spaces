...
> **This branch (`codex/spaces-pluginization`) is still under construction and acceptance. It is not a release.** Desktop isolation, safety, and license below still apply. The workbench (independent `127.0.0.1` supervisor, dedicated manager profile, iframe spaces) is documented in [docs/workbench.md](docs/workbench.md). Do not treat local builds or candidate packages as a published product or a way around the official CLI gate.
