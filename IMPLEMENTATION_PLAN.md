...
>
> 后续交付按 `tasks/plan.md` 与 `tasks/todo.md` 推进。以下保留早期实施记录。`web` 禁写隔离 patch，但明确升级允许更新官方基础插件。不能把本计划勾选记录替代当前的独立运行验收。协调升级和快照恢复曾作为 2026-09-08 新增范围；涉及修复、恢复、回滚或救援的产品要求已由 let it crash 取代，不再作为当前施工与发布门槛。历史事实不因此改写。

> 版本：1.0（2026-08-28）
> 前置阅读：`REQUIREMENTS.md`（需求与验收）、`docs/let-it-crash.md`（现行故障政策）、`web-dump-config.txt`（本机实测配置树）
...
1. NFR1 全面落实：所有写路径过一遍审计（备份、原子写、web 零写入）。提交前备份与原子写继续有效；提交后把旧环境写回去不属于本阶段目标。
2. NFR2 版本防护：启动前校验流程接入每一次 start；人为破坏 row id 的测试用例（验收 A8）。校验失败拒绝并说明，不自动改 patch。
3. 插件安装队列（FR5）：串行执行 `dsh plugin` 命令，UI 显示队列状态。安装失败结束该次操作并报错，不重试、不换版本。
4. 错误传播与失败展示：crashed 状态可见，含空间、阶段、已知原因或明确未知；启动超时是有界失败（端口探活超时 → 报错而非无限转圈）。用户可从常规菜单主动启动或重启，这是独立操作。不自动重启、不进入救援模式。
5. 设置页（FR6）。
6. 阶段 0 脚本接入 CI（GitHub Actions），作为发布门（NFR3）。

**完成判据**：验收 A1–A11 全绿（A11：失败可见且无抢救），CI 绿。不以恢复成功为通过条件。

历史记录：本阶段在 2026-09 曾按“崩溃恢复 / 一键重启”实施并勾选。该实现事实保留；自 2026-09-15 起，抢救成功不再是阶段出口。
