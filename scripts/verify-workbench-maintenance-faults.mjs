...
const DRIVER = join(root, 'scripts/workbench-fault-supervisor.mjs');
const WORKER_TS = join(root, 'scripts/workbench-fault-snapshot-worker.ts');
...
function parseFlags(argv) {
  ...
  let selfCheck = false;
  let httpOnly = true;
  let out = process.env.DSH_TEST_OUTPUT || join(root, '.sandbox', 'workbench-maintenance-faults');
  ...
  return { phase, selfCheck: selfCheck || phase === 'self-check', httpOnly, output: resolve(out), home: home.trim() ? resolve(home.trim()) : '' };
}
...
export async function runSelfCheck(outputDir = join(root, '.sandbox', 'workbench-maintenance-faults')) {
  ...
  for (const file of [
    join(root, 'scripts/verify-workbench-maintenance-faults.mjs'),
    DRIVER,
  ]) {
    const result = spawnSync(node, ['--check', file], { cwd: root, windowsHide: true, encoding: 'utf8' });
    if (result.status !== 0) fail(`syntax ${basename(file)}`, result.stderr || result.stdout);
    else pass(`syntax ${basename(file)}`);
  }
  ...
  try {
    ...
    if (check.status !== 0) fail('fault-worker-bundle', check.stderr || check.stdout);
    else pass('fault-worker-bundle', { path: workerJs });
  } catch (error) {
    fail('fault-worker-bundle', error);
  }
  ...
  try {
    ...
    if (result.status !== 0) fail('fault-supervisor-self-check', result.stderr || result.stdout);
    else if (!String(result.stdout).includes('fault-supervisor-self-check=ok')) fail('fault-supervisor-self-check', result.stdout);
    else pass('fault-supervisor-self-check', { stdout: String(result.stdout).trim() });
  } catch (error) {
    fail('fault-supervisor-self-check', error);
  }
...
async function bundleFaultWorker(output) {
  ...
  const outfile = join(output, 'fault-snapshot-worker.mjs');
  mkdirSync(output, { recursive: true });
  await build({
    bundle: true,
    format: 'esm',
...
async function runFaults(flags) {
  ...
  pass('plugin, view-bridge packed; fault worker bundled from original SnapshotStore');
  save();
...
async function phaseRestoreKill(ctx, row, pass) {
  ...
  try {
    ...
    row.coldRestart = {
      ...
      jobs: await packed.api('jobs').catch(error => ({ error: redact(error instanceof Error ? error.message : error) })),
    };
    ...
    const jobsAfter = await packed.api('jobs');
    const interruptedAfter = jobsAfter.find(item => item.id === interrupted.id);
    assert.ok(interruptedAfter, 'interrupted job disappeared');
    assert.notEqual(interruptedAfter.status, 'succeeded', 'unproven restore job must not be settled succeeded');
    ...
    pass(`restore-kill: recovered via ${recoveredBy}; data hash matches pre-restore; unproven job not succeeded; entry queryable`);
  } finally {
...
async function phaseUpgradeKill(ctx, row, pass) {
  ...
  try {
    ...
    const jobs = await packed.api('jobs');
    const interrupted = jobs.find(item => item.id === jobId) || jobs.find(item => item.status === 'recovery-required');
    ...
    if (interruptedAfter?.status === 'succeeded') {
      throw new Error('unproven upgrade job was settled succeeded');
    }
    row.jobsAfter = jobsAfter.map(item => ({ id: item.id, status: item.status, kind: item.kind, phase: item.phase }));
    await assertStableEntry(packed);
    ...
    pass(`upgrade-kill: recovered via ${recoveredBy}; original hash/runtime; unproven job not succeeded; entry up`);
  } finally {
