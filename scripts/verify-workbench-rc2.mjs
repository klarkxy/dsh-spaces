#!/usr/bin/env node
/**
 * Isolated official 0.1.5-rc.2 compatibility evidence. Fresh Home, empty
 * config, no model calls, no production ~/.dsh. Does not import
 ...
 * use `--from-default-profile web`. Do not clone shipped `web` onto itself.
 *
 ...
 * only this process's spawned PIDs.
 */
...
async function probeSupervisor(bin, home, env, artifacts) {
  ...
  try {
    ...
    if (state?.role === 'manager' && managerRunning && state.dshVersion === RC2 && !state.recoveryRequired) {
      ...
      pass('packed supervisor admitted a running manager on official live 0.1.5-rc.2');
    } else {
      result.refused = true;
      pass('product manager was refused or left in recovery on official 0.1.5-rc.2');
...
async function main() {
  ...
  if (product.admitted) {
    pass('product path admitted official 0.1.5-rc.2');
  ...
  report.scope = product.admitted
    ? 'official live 0.1.5-rc.2 product manager admitted; low-level CLI/bridge/guide wiring and Chromium experiment parent also proved'
    : 'low-level CLI / view-bridge / guide-only plugin on official live 0.1.5-rc.2 plus Chromium experiment parent; manager not admitted; full product unvalidated';
