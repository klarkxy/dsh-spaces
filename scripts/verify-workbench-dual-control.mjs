...
async function main() {
  ...
  try {
    ...
    const takeoverCopy = await electronPage.getByText(/接管|Take over|移交|Hand off/).count();
    if (takeoverCopy) {
      throw new Error('desktop still shows takeover/hand-off UI while web owns this Home');
    }
    pass('desktop has no takeover or hand-off chip while web owns; screenshot captured');
