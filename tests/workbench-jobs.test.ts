...
test("result and view projection drop path, token, and non-loopback origins", async () => {
  ...
  await store.submit(startCmd(), "proj-1", async () => ({
    ...
    view: {
      ...
      entryOrigin: "http://127.0.0.1:3211", entryPath: "/workbench/view/alpha",
      channel: "ch-1",
    },
  }));
  await store.whenIdle();
  ...
  await store.submit(startCmd("beta"), "proj-2", async () => ({
    ...
    view: {
      ...
      entryOrigin: "http://127.0.0.1:3211", entryPath: "//127.0.0.1/workbench/view/beta",
      channel: "ch-2",
    },
  }));
  await store.whenIdle();
  ...
  await store.submit(startCmd("gamma"), "proj-3", async () => ({
    ...
    view: {
      ...
      entryOrigin: "http://127.0.0.1:3211", entryPath: "/workbench/view/gamma",
      channel: "ch-3",
    },
  }));
  await store.whenIdle();
  ...
  await store.submit(startCmd("delta"), "proj-4", async () => ({
    ...
    view: {
      ...
      generation: 3,
      origin: "http://127.0.0.1:3210",
      entryOrigin: "http://127.0.0.1:3211", entryPath: "/workbench/view/delta",
      channel: "ch-4",
    },
  }));
  await store.whenIdle();
